"""MD2 参考实现：用与 JS 完全相同的 S 表，判断是表错还是代码错"""
import hashlib, sys
...
print("S 表长度:", len(S), "唯一值数:", len(set(S)))


def md2(msg: bytes) -> bytes:
    L = 16
    n = len(msg) // L + 1
    pad = n * L - len(msg)
    m = bytearray(msg) + bytes([pad]) * pad
    X = bytearray(48)
    C = bytearray(16)
...
for k, v in V.items():
    g = md2(k).hex()
    print(k, g, v, "OK" if g == v else "FAIL")
